...
## 3. Các loại Log quan trọng cần biết

Trong Linux, mọi dấu vết đều nằm ở thư mục `/var/log`. Tùy tình huống mà ta lôi loại log tương ứng ra dùng:

| Loại Log           | Địa chỉ file                 | Ứng dụng thực tế                                             |
|:------------------ |:---------------------------- |:------------------------------------------------------------ |
| **Bảo mật & Sudo** | `/var/log/auth.log`          | Theo dõi đăng nhập, lỗi mật khẩu, dùng lệnh `sudo`.          |
| **Hệ thống chung** | `/var/log/syslog`            | Log tổng hợp mọi sự kiện, dùng khi lỗi không rõ nguyên nhân. |
| **Web Server**     | `/var/log/apache2/error.log` | Xem lỗi code web (khi hiện lỗi 500, trang trắng...).         |
| **Nhân hệ thống**  | `/var/log/kern.log`          | Kiểm tra lỗi phần cứng, lỗi hết RAM (OOM Killer).            |
| **Khởi động**      | `dmesg` (lệnh)               | Kiểm tra nhận diện thiết bị ngay khi vừa bật máy.            |
